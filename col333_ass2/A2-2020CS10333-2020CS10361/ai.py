...
class AIPlayer:
    ...
    def expectimax_max_val(self,state,height):
        k=0
        if self.player_number==1:
            k=2
        else :
        ...
        parameter=-np.inf
        for act in moves:
            parameter=max(parameter,self.expectimax_val(self.expectimax_change_state(copy.deepcopy(state),act,self.player_number),height-1))
        return parameter
    def expectimax_val(self,state,height):
        k=0
        if self.player_number==1:
            k=2
        else :
        ...
        if  height==0: 
            ...
            a=-np.inf
            for act in moves:
                s=self.expectimax_change_state(copy.deepcopy(state),act,k)
                z1=get_pts(k,state[0])-get_pts(k,s[0])
                z2=get_pts(self.player_number,s[0])-get_pts(k,s[0])
                z3=get_pts(self.player_number,s[0])-get_pts(self.player_number,state[0])
        ...
        p=1/len(moves)
        for act in moves:
            parameter=parameter+(p*self.expectimax_max_val(self.expectimax_change_state(copy.deepcopy(state),act,k),height-1))
        return parameter
    ...
    def get_expectimax_move(self, state: Tuple[np.array, Dict[int, Integer]]) -> Tuple[int, bool]:
        ...
        step=moves[0]
        for act in moves:
            z=self.expectimax_val(self.expectimax_change_state(copy.deepcopy(state),act,self.player_number),height-1)
            if z>parameter:
                parameter=z
                step=act
