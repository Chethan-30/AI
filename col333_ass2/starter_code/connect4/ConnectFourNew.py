...
from connect4.players.Online import OnlinePlayer
...
class Game:
    def __init__(self, player1, player2, time: int, board_init: np.array, m: int, n: int, popout_moves: int):
        ...
        for j in range(n):
            ...
            for i in range(m):
                col = i * 75
                c = board[i][j]
                column.append(self.c.create_oval(row, col, row + 75, col + 75, fill=self.colors[c]))
    ...
    def threaded_function(self, arg):
        ...
        for i in range(arg):
            ...
            sleep(0.01)
            if self.game_over:
                if self.players[self.current_turn].type == 'online':
                    self.players[self.current_turn].send_move(("e", True))
                elif self.players[int(not self.current_turn)].type=="online":
                    self.players[int(not self.current_turn)].send_move(("e", True))
    ...
    def make_move(self):
        ...
        if not self.game_over:
            ...
            if self.players[int(not self.current_turn)].type == 'online':
                if self.players[int(not self.current_turn)].send_move(action):
                    self.game_over = True
...
def main(player1: str, player2: str, init_fine_name: str, time: int, port: int):
    ...
    m, n = board.shape
    if player1 == 'online' or player2 == 'online':
        other = player1 if player2 == 'online' else player2
        player = OnlinePlayer(port)
        while player.player_number==None:
            sleep(0.1)
        [m, n, num_pop_outs, time] = player.message[1:]
        board = np.zeros((m, n), dtype=int)
        if player.player_number==1:
            Game(player, make_player(other, 2), time, board, m, n, num_pop_outs)
        else:
            Game(make_player(other, 1), player, time, board, m, n, num_pop_outs)
    else:
        Game(make_player(player1, 1), make_player(player2, 2), time, board, m, n, num_pop_outs)


if __name__ == '__main__':
    player_types = ['ai', 'random', 'human', 'online']
    ...
    parser.add_argument('--port', type=int, default=None, help='port of server')
    args = parser.parse_args()
    main(args.player1, args.player2, args.start_file, args.time, args.port)
